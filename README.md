...
| `max_cached_reviews` | 每个画师本地缓存的最大评价条数（用于去重） | `500` |
| `enable_auto_push` | 是否启用定时自动推送 | `true` / `false` |
| `push_cron` | 定时推送的 Cron 表达式（5段） | `*/30 * * * *` |
| `push_target` | 全局推送目标（可选）。可填写完整的 `unified_msg_origin`，例如 `aiocqhttp:group_123456`。如果只填写数字群号，会自动转换为 `aiocqhttp:group_` 格式（兼容旧版）。 | |
...
### v1.0.7 (2026-04-03)
- 🐛 **订阅文件并发竞态**：引入 _update_subscriptions() 原子更新，读-改-写全部在锁内完成。
- 🚀 **推送异常中断整批**：asyncio.gather(..., return_exceptions=True) 并记录失败，不影响其他推送。
- 🛡️ **指纹哈希碰撞**：改用 hashlib.sha256 生成稳定指纹。
- 🔧 **缓存依赖 API 顺序**：保存前按时间字段显式排序（降序）。
- 📝 **HTML/ Markdown 注入风险**：增加 _html_escape() 转义用户内容；头像 URL 校验协议。
- 📝 *未使用的导入**：移除 re, Tuple。

### v1.0.6 (2026-04-03)
- 🐛 **修复调度器重启问题**：解决插件重载后自动推送无法重新启动的 bug。
- 🚀 **新增配置项**：`max_cached_reviews` 可自定义每个画师缓存的最大评价条数（默认 500）。
- 🛡️ **命令解析增强**：使用 `get_plain_text()` 处理带 @ 前缀的消息，提高兼容性。
- 🔧 **移除未使用依赖**：`requirements.txt` 中不再需要 `requests`。
- 📝 **文档更新**：补充新配置项说明。
...
- 🛡️ **健壮性提升**：画师ID格式校验、错误提示细化、`max_reviews_display` 限制在1-50之间。
- 🔧 **兼容性**：自动将6段Cron表达式转换为5段。
- 🧊 **冷启动保护**：首次运行只保存评价，不推送。
